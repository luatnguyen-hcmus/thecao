	...
	<div class="widget">
		...
		<table cellpadding="0" cellspacing="0" width="100%" class="sTable mTable myTable" id="checkAll">
			...
			<thead class="filter"><tr><td colspan="9">
				<form class="list_filter form" action="<?php echo admin_url('typecardbuy') ?>" method="get">
					<table cellpadding="0" cellspacing="0" width="80%"><tbody>
						<tr>
							...
							<input type="reset" class="basic" value="Reset" onclick="window.location.href = '<?php echo admin_url('typecardbuy') ?>'; ">
							</td>
				    ...
				    <tr class='row_<?php echo $row->id ?>'>
						...
						<td class="textC">
							<?php foreach($row->subs as $subss):?>
								<b><?=$subss->hunress_buy?> %</b>
							<?php endforeach?>
						 </td>
						<td class="textC">
							<?= $row->status_buy==1?'Hiển thị':'Không hiển thị' ?>
						</td>
						<td class="textC">
							<?= $row->featured_buy==1?'Hoạt động':'Bảo trì' ?>
						</td>
						<td class="option textC">
							<a href="<?php echo admin_url('typecardbuy/edit/'.$row->id) ?>" title="Chỉnh sửa" class="tipS">
								<img src="<?php echo public_url('admin')?>/images/icons/color/edit.png" />
							</a>
							<a href="<?php echo admin_url('typecardbuy/delete/'.$row->id) ?>" title="Xóa" class="tipS verify_action" >
